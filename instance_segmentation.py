...
parser.add_argument("-i", "--image", default='images/scotty.jpg', type=str,
	help="Path to input image")
parser.add_argument("-m", "--model", default='mask_rcnn_inception_v2_coco_2018_01_28',
	help="Optionale. TensorFlow model used to perform the instance segmentation")
...
MODEL_NAME = args['model'] #'mask_rcnn_inception_v2_coco_2018_01_28'
...
def main():
    if Path(args['image']).exists() is not True:
    ...
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:    
            ...
            output = run_inference_for_single_image(image, sess, detection_graph)
            processed_image = visualize_results(image, output)
            processed_image = cv2.cvtColor(processed_image, cv2.COLOR_RGB2BGR)
            cv2.imshow('Original image', img)
            cv2.imshow('Instance Segmented image', processed_image)
            cv2.waitKey(1) & 0xFF
            input("Image segmentation completed: press the 'Enter' button to exit")
